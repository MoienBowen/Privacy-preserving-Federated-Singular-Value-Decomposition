 ...
 * .
 * Since /dev/random is one of the sources of randomness in prime generation,
 * the program may block. In that case, you have to wait or move your mouse to
 * feed /dev/random with fresh randomness.
 */
int paillier_keygen(paillier_public_key *pub, paillier_private_key *priv,
                    mp_bitcnt_t len) {
  ...
  crt_exponentiation(temp, g, priv->lambda, priv->lambda, priv->p2invq2,
                     priv->p2, priv->q2);
 ...
 * Encryption benefits from the fact that g=1+n, because (1+n)^m = 1+n*m mod
 * n^2.
 */
int paillier_encrypt(mpz_t ciphertext, mpz_t plaintext,
 ...
 * The exponentiation is calculated using the CRT, and exponentiations mod p^2
 * and q^2 run in their own thread.
 *
 */
int paillier_decrypt(mpz_t plaintext, mpz_t ciphertext,
                     paillier_private_key *priv) {
  DEBUG_MSG("computing plaintext\n");
  // compute exponentiation c^lambda mod n^2
  crt_exponentiation(plaintext, ciphertext, priv->lambda, priv->lambda,
 ...
 * For example, given the ciphertexts c1 and c2, encryptions of plaintexts m1
 * and m2, the value c3=c1*c2 mod n^2 is a ciphertext that decrypts to m1+m2 mod
 * n.
 */
int paillier_homomorphic_add(mpz_t ciphertext3, mpz_t ciphertext1,
                             mpz_t ciphertext2, paillier_public_key *pub) {
  mpz_t n2;
...
/**
 * "Multiplies" a plaintext with a constant homomorphically by exponentiating
 * the ciphertext modulo n^2 with the constant as exponent. For example, given
 * the ciphertext c, encryptions of plaintext m, and the constant 5, the value
 * c3=c^5 n^2 is a ciphertext that decrypts to 5*m mod n.
 */
int paillier_homomorphic_multc(mpz_t ciphertext2, mpz_t ciphertext1,
                               mpz_t constant, paillier_public_key *pub) {
  mpz_t n2;
...
int paillier_redandomise(mpz_t ciphertext2, mpz_t ciphertext1,
                         paillier_public_key *pub) {
  mpz_t r, n2;
  ...
  mpz_powm(r, r, pub->n, n2);

  mpz_mul(ciphertext2, ciphertext1, r);
