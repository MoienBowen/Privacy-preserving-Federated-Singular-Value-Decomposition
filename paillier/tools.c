...
/**
 * Generate a random number using /dev/random for the first 128 bits and then
 * /dev/urandom. This guarantees that there is enough entropy in the pool and
 * that it is safe to use /dev/urandom. Since /dev/random is used, if entropy is
 * insufficient the program will block. In that case, it is necessary to feed
 * /dev/random with entropy, for example by moving the mouse.
 */
...
/**
 * Generate a random prime number using /dev/random and /dev/urandom as a source
 * of randomness.
 * @see gen_random
 */
 ...
 * @ingroup Tools
 *
 * This function reduces the input modulo the given modulus, and performs a
 * modular exponentiation. It is intended to be run in a pthread.
 *
 * @param[in,out] args arguments for the exponentiation, a pointer to an
 * exp_args, with
...
void *do_exponentiate(void *args) {
  ...
  mpz_powm(args_struct->result, basis_reduced, args_struct->exponent,
           args_struct->modulus);
